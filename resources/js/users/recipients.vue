<template>
    <div class="container">
                                ...
                                <div class="card mx-2 my-2">
                                    <div class="card-body">
                                        <table class="table table-responsive-sm table-hover">
                                            ...
                                            <tbody>
                                                <tr v-for="recipient in recipients.data" :key="recipient.id" class="text-capitalize">
                                                    <td>{{ recipient.id }}</td>
                                                    <td>{{ recipient.firstname }}</td>
                                                    <td>{{ recipient.middlename }}</td>
                                                    <td>{{ recipient.lastname }}</td>
                                                    <td>{{ recipient.email }}</td>
                                                    <td>{{ recipient.created_at | myDate }}</td>
                                                    <td>

                                                        <button type="button" class="btn btn-sm btn-danger" @click="archiverecipient(recipient.id)">
                                    ...
                                    <div class="card-footer">
                                        <pagination :data="recipients" @pagination-change-page="getResults">
                                        </pagination>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <table class="table table-responsive-sm table-hover">
                                            ...
                                            <tbody>
                                                <tr v-for="archive in archives.data" :key="archive.id" class="text-capitalize">
                                                    ...
                                                    <td>
                                                        <button type="button" class="btn btn-sm btn-success" @click="restorerecipient(archive.id)">               
        ...
        <div class="row">
            <div class="modal fade" id="recipientmodal" tabindex="-1" role="dialog" aria-labelledby="addmodalLabel" aria-hidden="true">
                <div class="modal-dialog  modal-dialog-centered" role="document">
                    <div class="modal-content">
                        ...
                        <form @submit.prevent="editmode ? updaterecipient() : createrecipient()">
...
<script>
    export default {
        ...
        methods: {
            getResults(page = 1) {
                axios.get('api/recipient?page=' + page)
                    .then(response => {
                        this.recipients = response.data;
                    });
            },
            getResults2(page = 1) {
                axios.get('api/recipientarchives?page=' + page)
                    .then(response => {
                        this.recipientsarchives = response.data;
                    });
            },
            loaddata(){
                axios.get('api/recipient').then(({ data }) => (this.recipients = data ));
            },
            loadarchiveds(){
                axios.get('api/recipientarchives').then(({ data }) => (this.archives = data ));
            },
            newmodal(){
                this.editmode = false;
                this.form.reset();
            ...
            updaterecipient(id) {
                this.$Progress.start();
                this.form.put('api/recipient/'+this.form.id).then(() => {
                    Fire.$emit('success');
                    $('#recipientmodal').modal('hide');
                        toast({
                            type: 'success',
                            title: 'recipient Created in Successfully'
                        });
            ...
            archiverecipient(id) {
                ...
                confirmButtonText: 'Yes, delete it!'
                    ...
                    if (result.value){
                        this.form.delete('api/recipient/'+id).then(()=>{
                            swal(
                                'Archived!',
            ...
            restorerecipient(id){
                ...
                confirmButtonText: 'Yes, delete it!'
                    ...
                    //Send request to the server
                    if (result.value){
                        axios.get('api/recipientrestore/'+id).then(()=>{
                            swal(
                                'Restored!',
                                'Recipient has been restore.',
                                'success'
                            )
                            Fire.$emit('success');
                        }).catch(()=>{
