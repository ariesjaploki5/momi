<template>
    <div class="container">
                                        ...
                                        <table class="table table-responsive-sm table-hover">
                                            ...
                                            <tbody>
                                                <tr v-for="doctor in doctors.data" :key="doctor.id">
                                                    <td>{{ doctor.id }}</td>
                                                    <td class="text-capitalize">{{ doctor.firstname }}</td>
                                                    <td class="text-capitalize">{{ doctor.middlename }}</td>
                                                    <td class="text-capitalize">{{ doctor.lastname }}</td>
                                                    <td>{{ doctor.email }}</td>
                                                    <td>{{ doctor.created_at | myDate }}</td>
                                                    <td>
                                                        <button type="button" class="btn btn-sm btn-primary" @click="editmodal(doctor)" >               
                                                            <i class="fa fa-edit"></i>
                                                            Edit
                                                        </button>
                                                         <button type="button" class="btn btn-sm btn-danger" @click="archivedoctor(doctor.id)">               
                                                            <i class="fas fa-trash"></i>
                                                            Delete
                                                        </button>
                                                        <!-- <button type="button" class="btn btn-sm btn-warning" @click="archivedoctor(doctor.id)">
                                        ...
                                        <table class="table table-responsive-sm table-hover">
                                            ...
                                            <tbody>
                                                <tr v-for="archive in archives.data" :key="archive.id" class="text-capitalize">
                                                    ...
                                                    <td>
                                                        <button type="button" class="btn btn-sm btn-success" @click="restoredoctor(archive.id)">               
        ...
        <div class="row">
            <div class="modal fade" id="doctormodal" tabindex="-1" role="dialog" aria-labelledby="addmodalLabel" aria-hidden="true">
...
<script>
    export default {
        ...
        methods: {
            getResults(page = 1) {
                axios.get('api/doctor?page=' + page)
                    .then(response => {
                        this.doctors = response.data;
                    });
            },
            getResults2(page = 1) {
                axios.get('api/doctorarchives?page=' + page)
                    .then(response => {
                        this.doctorsarchives = response.data;
                    });
            },
            loaddata(){
            ...
            loadarchiveds(){
                axios.get('api/doctorarchives').then(({ data }) => (this.archives = data ));
            },
            newmodal(){
                this.editmode = false;
                this.form.reset();
            ...
            createdoctor() {
                ...
                this.form.post('api/doctor').then(() => {
                    ...
                    $('#doctormodal').modal('hide');
                        toast({
                            type: 'success',
                            title: 'Doctor Created in Successfully'
                        });
            ...
            updatedoctor(id) {
                this.$Progress.start();
                this.form.put('api/doctor/'+this.form.id).then(() => {
                    Fire.$emit('success');
                    $('#doctormodal').modal('hide');
                        toast({
                            type: 'success',
            ...
            restoredoctor(id){
                ...
                confirmButtonText: 'Yes, restore it!'
                    ...
                    //Send request to the server
                    if (result.value){
                        axios.get('api/doctorrestore/'+id).then(()=>{
                            swal(
                                'Restored!',
                                'Doctor Account has been restore.',
                                'success'
                            )
                            Fire.$emit('success');
                        }).catch(()=>{
            ...
            forcedeletedoctor(id){
                ...
                confirmButtonText: 'Yes, delete it!'
                    ...
                    //Send request to the server
                    if (result.value){
                        axios.delete('api/doctorforcedelete/'+id).then(()=>{
                            swal(
                                'Deleted!',
                                'Doctor account has been deleted.',
                                'success'
